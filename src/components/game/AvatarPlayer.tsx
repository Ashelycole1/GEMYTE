...
export default function AvatarPlayer({ gender = 'male' }: { gender?: 'male' | 'female' }) {
  ...
  return (
    <RigidBody ref={rigidBodyRef} colliders={false} mass={1} type="dynamic" position={[0, 5, 0]} lockRotations enabledRotations={[false, false, false]} friction={0}>
      ...
      <group ref={groupRef}>
        ...
        <group ref={leftArmRef} position={[gender === 'male' ? 0.45 : 0.35, 1.4, 0]}>
          <mesh position={[0, -0.3, 0]}><capsuleGeometry args={[0.12, 0.5, 16, 16]} /><meshStandardMaterial color={shirtColor} /></mesh>
          <mesh position={[0, -0.7, 0]}><sphereGeometry args={[0.14, 16, 16]} /><meshStandardMaterial color="#ffedd5" /></mesh>
        </group>
        ...
        <group ref={leftLegRef} position={[0.2, 0.7, 0]}>
          <mesh position={[0, -0.3, 0]}><capsuleGeometry args={[0.15, 0.5, 16, 16]} /><meshStandardMaterial color={pantsColor} /></mesh>
          <mesh position={[0, -0.7, 0.05]}><boxGeometry args={[0.2, 0.15, 0.3]} /><meshStandardMaterial color="#000000" /></mesh>
        </group>
